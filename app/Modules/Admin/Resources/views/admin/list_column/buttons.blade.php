@foreach ($li['buttons'] as $button)
	...
	@if ($button == 'show')
		<a class="btn btn-info btn-sm" href="{{ URL::route('admin.'.$module.'.show', $item->id) }}?{{ $_SERVER['QUERY_STRING'] }}">
			<i class="fa fa-eye "></i>  
		</a>
	@endif 

	@if ($button == 'edit')
		<a class="btn btn-warning btn-sm" href="{{ URL::route('admin.'.$module.'.edit', $item->id) }}?{{ $_SERVER['QUERY_STRING'] }}">
