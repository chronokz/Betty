@foreach ($li['buttons'] as $button)
	...
	@if ($button == 'show')
		<a class="btn btn-info btn-sm" href="{{ URL::route('admin.'.$module.'.show', $item->id) }}?{{ $_SERVER['QUERY_STRING'] }}" data-toggle="tooltip" title="Посмотреть">
			<i class="fa fa-eye "></i>  
		</a>
	@endif 

	@if ($button == 'edit')
		<a class="btn btn-warning btn-sm" href="{{ URL::route('admin.'.$module.'.edit', $item->id) }}?{{ $_SERVER['QUERY_STRING'] }}" data-toggle="tooltip" title="Редактировать">
	...
	@if ($button == 'delete')
		{{ Form::open(array('url' => URL::route('admin.'.$module.'.destroy', $item->id), 'class' => 'delete-destroy', 'method' => 'DELETE')) }}
			<a class="btn btn-danger btn-sm submit" data-toggle="tooltip" title="Удалить">
